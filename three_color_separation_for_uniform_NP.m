...
for kk = 1:size(NP_all,2)
    ...
    [maxValues3(:,kk), colIndices3(:,kk)]= max(LogL(:,:,3,kk),[],2);
    correct3(kk) = sum(colIndices3(:,kk) == 3) / length(colIndices3(:,kk)) * 100;
    wrong3(kk) = sum(colIndices3(:,kk) ~= 3) / length(colIndices3(:,kk)) * 100;
end
...
hold on
plot(NP_all,smooth(correct3,0.5,'loess'));
xlim([0,5000])
ylim([0,110])
